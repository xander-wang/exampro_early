			...
			<form method="post" action="">
				...
				<div class="well">
					<c:forEach items="${list}" var="fun">
						<c:if test="${fun.getFunpid() == -1}">
							<div class="toc">
								<h3>
									<input type="checkbox" onclick="selectparent(this)" ma="${fun.getFunid() }" name="ckrr" value="${fun.getFunid}"
										   <c:if test="${fun.isRr() == true}">checked="checked"</c:if>/>
								${fun.getFunname()}
								</h3>
								<ul>
									<c:forEach items="${list}" var="child">
										<c:if test="${child.getFunpid() == fun.getFunid()}">
											<li>
												<input type="checkbox" onclick="selectchild(this)" parent="${fun.getFunid() }" name="ckrr" value="${child.getFunid() }"
													   <c:if test="${child.isRr() == true}">checked="checked"</c:if>/>
												${child.getFunname() }
											</li>
										</c:if>
									</c:forEach>
